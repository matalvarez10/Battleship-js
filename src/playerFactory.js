...
import { players } from ".";
let valuesToExclude = [];
const allGridsContainer = document.getElementById("game-container");
  ...
  const clickHit = (gridReference, gameboardRef, allShips, nombreBoard) => {
    ...
    if (validMove(x, y, gameboardRef) === true) {
      ...
      if (["C", "B", "D", "S", "P"].includes(gameboardRef[x][y])) {
        const shipHit = allShips.filter(
          (element) => element.getId() === gameboardRef[x][y]
        );
        shipHit[0].hit();
        if (checkGameOver(allShips)) {
          allGridsContainer.style.pointerEvents = "none";
